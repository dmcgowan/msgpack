...
func (et *extensionTester) encodeCounterStruct(v reflect.Value) ([]byte, error) {
	counter, ok := v.Interface().(*counterStruct)
	if !ok {
		return nil, fmt.Errorf("Unexpected encode type: %T", v.Interface())
	}
	return []byte{et.id, counter.decodeCount, counter.encodeCount + 1}, nil
}

func TestEncode(t *testing.T) {
	extensionTester1 := &extensionTester{1}
	config1 := NewExtensions()
	config1.AddExtension(1, reflect.TypeOf(&counterStruct{}), nil, extensionTester1.encodeCounterStruct, extensionTester1.decodeCounterStruct)
	extensionTester2 := &extensionTester{2}
	config2 := NewExtensions()
	config2.AddExtension(1, reflect.TypeOf(&counterStruct{}), nil, extensionTester2.encodeCounterStruct, extensionTester2.decodeCounterStruct)
